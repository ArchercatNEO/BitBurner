export function spiralize(data: number[][]): null | number[] {
    ...
    while (flatMap.length < area) {
        for (leftLimit++; x < rightLimit; x++) { push() } //right
        if (isInvalid()) { break }
        for (upLimit++; y < downLimit; y++) { push() } //down
        if (isInvalid()) { break }
        for (rightLimit--; x > leftLimit; x--) { push() } //left
        if (isInvalid()) { break }
        for (downLimit--; y > upLimit; y--) { push() } //up
    }
    ...
    function push() {flatMap.push(data[y][x])}
